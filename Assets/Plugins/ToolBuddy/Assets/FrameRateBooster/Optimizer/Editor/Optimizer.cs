...
namespace ToolBuddy.FrameRateBooster.Optimizer
{
	public class Optimizer : IPostBuildPlayerScriptDLLs
	{
		...
		{
			if (PlayerSettings.GetScriptingBackend(NamedBuildTarget.FromBuildTargetGroup(report.summary.platformGroup)) !=
		...
		{
			if (PlayerSettings.GetScriptingBackend(NamedBuildTarget.FromBuildTargetGroup(BuildPipeline.GetBuildTargetGroup(target))) ==
			    ScriptingImplementation.IL2CPP) return;
		...
		private static void OptimizeBuild(BuildTarget target, string buildDirectoryPath)
		{
			...
			string optimizationsAssemblyPath;
			{
				...
				if (!GetUniqueTargetAssembly(optimizationsAssemblyName, allAssembliesPaths, buildDirectoryPath,
				                             "the assembly containing the optimizations", out optimizationsAssemblyPath))
					return;
			}
			...
			var optimizedMethodsCount = Optimize(optimizationsAssemblyPath, targetAssemblyPath, optimizationInOwnAssembly,
		                           ...
		                           bool   deleteOptimizationsAssembly, bool   trimOptimizationsAssembly)
		{
			...
			using ModuleDefinition optimizedModuleDefinition =
				ModuleDefinition.ReadModule(optimizationsAssemblyPath,
				                            new ReaderParameters { ReadWrite = trimOptimizationsAssembly });
			using (ModuleDefinition originalModule =
			       ModuleDefinition.ReadModule(targetAssemblyPath, new ReaderParameters { ReadWrite = true }))
			{
					...
					if (optimizedType.Namespace == optimizedNameSpace)
					{
						...
						foreach (MethodDefinition optimizedMethod in optimizedType.Methods)
						{
							...
							foreach (Instruction instruction in optimizedMethod.Body.Instructions)
							{
								...
								{
									case FieldReference fieldReference when
										fieldReference.DeclaringType.Namespace == optimizedNameSpace:
									...
									case MethodReference methodReference:
										methodReference.DeclaringType =
											originalModule.ImportReference(methodReference.DeclaringType.Resolve());
